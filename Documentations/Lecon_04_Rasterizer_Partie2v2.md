Lecon_04_Rasterizer_Partie2.md
...
|---|---|---|---|---|
| **19** | Champ ajouté à `FragmentContext`, oublié dans `DrawTriangle` | écran noir uniforme (`1/0 = inf`) | Structure remplie ailleurs qu'à sa déclaration | ✅ corrigé |
| **20** | **Backface culling inversé depuis la L05** | mode `Depth` (couleur à sens géométrique) | Flip Y du viewport : front-face = aire **négative** | ✅ corrigé |
| **21** | **124 trous sur les coutures du clipping** | `Test_ClipCoverage` (comptage) | Intersection calculée dans deux sens différents | ✅ corrigé |
| **22** | `EdgeFunction` dédoublée (surcharge modifiée sans le noyau) | audit du dépôt | Délégation coupée → deux formules | ✅ corrigé |
| **23** | `ParseCamera` : `out_activeCamera != NULL_ENTITY;` | audit du dépôt | Comparaison au lieu d'affectation (warning C4553) | ✅ corrigé (22/08) |
| **24** | `ShadeFragment_Barycentric` contourne `AsFragmentContext` | audit du dépôt | `static_cast` direct → parade du bug n°17 désactivée | ✅ déjà corrigé — vérifié le 22/08 |
| **25** | FPS + Follow actifs simultanément sur la même caméra | dump des contrôleurs | Exclusion promise par un commentaire, jamais implémentée | ✅ corrigé (22/08) — invariant `CheckControllerExclusivity` |
| **26** | Clés JSON écrites mais jamais lues (`active`/`priority` dans `CameraFPS`) | `JsonReader::WarnUnread` | Aucun avertissement sur les clés inconnues | ⏳ ouvert |
| **27** | `SDL_LockTexture` « Invalid call » au redimensionnement | test de resize | Texture recréée pendant qu'une frame est en cours | ⏳ ouvert |
 
### 11.1 Correctifs post-clôture (22/08/2026)
 
Trois éléments restaient ouverts à la clôture de cette leçon (§13, "Bugs 23, 24, 25 du journal"). Audit de suivi :
 
**Bug 23 — `ParseCamera` (`Scene/Serializer.cpp`).** La sélection de la caméra active comparait correctement `out_activeCamera != NULL_ENTITY` dans la condition, mais la branche gagnante écrivait `out_activeCamera = NULL_ENTITY;` au lieu de `out_activeCamera = entity;`. Résultat : dès qu'une caméra active était trouvée, `out_activeCamera` était **réinitialisée à `NULL_ENTITY`** plutôt que de recevoir l'entité gagnante — la scène pouvait charger plusieurs caméras actives sans qu'aucune ne soit jamais retenue comme caméra courante.
 
```cpp
// Avant
if (!current || c.m_priority >= current->m_priority)
    out_activeCamera = NULL_ENTITY;   // ← BUG : efface la sélection au lieu de l'écrire
 
// Après
if (!current || c.m_priority >= current->m_priority)
    out_activeCamera = entity;        // ← la caméra courante, priorité la plus haute gagne
```
 
**Bug 24 — `ShadeFragment_Barycentric` (`Rendering/Fragment.cpp`).** À la relecture, la fonction passe déjà par `AsFragmentContext(userData)` — le `static_cast` direct signalé dans le journal n'est plus présent dans le dépôt. **Aucun changement nécessaire** ; la parade du bug n°17 (assertion `magic`) est bien active sur ce fragment. Marqué corrigé.
 
**Bug 25 — Exclusivité FPS/Follow (`Scene/System.hpp` / `System.cpp`).** `FPSControllerComponent` et `CameraFollowComponent` écrivent tous les deux dans `TransformComponent::m_local` ; rien ne détectait le cas où les deux étaient actifs simultanément sur la même entité (le second système écrase alors le premier, silencieusement). Ajout d'une fonction de diagnostic à appeler chaque frame, dans le même esprit que `CheckSceneInvariants` :
...
Ce n'est pas une correction du *symptôme* (les deux composants restent activables séparément — `SwitchCameraMode` respecte déjà l'exclusion quand on passe par elle) mais une **assertion qui rend l'incohérence bruyante** dès qu'un chemin de code contourne `SwitchCameraMode` en touchant directement `m_isEnabled`. Cohérent avec le principe méthodologique n°6 de cette leçon : une invariant non vérifiée n'est pas prouvée.
...
| ~~Bugs 23, 24, 25 du journal~~ | **Corrigés le 22/08/2026 — voir §11.1** |
| Bugs 26, 27 du journal | Non corrigés, reportés |
